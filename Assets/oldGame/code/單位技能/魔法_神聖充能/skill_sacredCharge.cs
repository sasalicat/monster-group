...
public class skill_sacredCharge : CDSkill {
    public const int HEALING_POINT = 20;
    ...
    public override void trigger(Dictionary<string, object> args)
    {
        ...
        if (!(bool)args["miss"])
        {
            foreach (BasicControler traget in tragets)
            {
                ...
                if(traget.data.Now_Life == traget.data.Now_Max_Life)
                {
                    foreach(CDSkill skill in ((BasicControler)traget).skillBelt.ActiveSkills)
                    {
                        skill.timeLeft -= 1.0f;
                    }
                }
