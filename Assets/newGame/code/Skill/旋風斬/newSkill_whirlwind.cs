...
public class newSkill_whirlwind : dynamicSkill
{
    ...
    public override unitControler[] getTragets(Environment env)
    {
        ...
        else
        {
            ...
            return getAliveEnemy((ChessBoard)env).ToArray();
        }
    }

    public override SkillInf Inf()
    {
        return new SkillInf_v2(this, false, true, false, false, new List<string>() { "damage"});
    }
    ...
    public override void trigger(Dictionary<string, object> args)
    {
        ...
        if (tragets.Length > 0)
        {
            ...
            GameObject effObj = resourcePool[prefabNames[0]];
            GameObject expro = resourcePool[prefabNames[1]];
            Dictionary<string, object> dictEff = new Dictionary<string, object>();
            dictEff["creater"] = owner;
            closeupStage.main.display_effect(effObj, dictEff, false);
            foreach (comboControler traget in tragets)
            {
                ...
                if (!missDict[traget] && !traget.data.Dead)
                {
                    ...
                    Damage_v2 d = createDamage((int)(owner.data.Now_Attack*0.5f), Damage.KIND_PHYSICAL, args);
